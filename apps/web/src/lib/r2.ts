...
function getR2Config() {
  ...
  const bucketName = process.env.R2_FILE_BUCKET;
  const publicBaseUrl = process.env.NEXT_PUBLIC_R2_FILE_BASE_URL;
...
export async function uploadFileToR2(
  ...
  if (!config) {
    return {
      message:
        "R2 存储尚未配置，请检查 R2_ACCOUNT_ID / R2_ACCESS_KEY_ID / R2_SECRET_ACCESS_KEY / R2_FILE_BUCKET / NEXT_PUBLIC_R2_FILE_BASE_URL",
      ok: false
    };
  }
